...
# I run MobileNetV2 on webcam frames, with confidence gating, ROI, smoothing, and a still-frame "test.py" check.
...
def main():
    ...
    ap.add_argument("--img-size", type=int, default=160, help="must match training (you used 160)")
    ...
    # transforms — EXACTLY like your training (no normalization)
    ...
    while True:
        ...
        # centered ROI (helps reduce background bias)
        if use_roi:
        ...
        # preprocess (BGR->RGB handled by PIL inside transforms)
        ...
        # confidence gating (same spirit as 'test.py' honesty)
        ...
        if debug:
            # show top-2 for quick intuition
        ...
        if k in (ord('q'), 27):  # quit
            break
        elif k == ord('r'):      # ROI toggle
            use_roi = not use_roi
        elif k == ord('c'):      # clear smoothing
            buf.clear(); ema_prob = None; print("Smoothing cleared.")
        elif k == ord('d'):      # debug
            debug = not debug
        elif k == ord('s'):      # save current frame
            pth = out_dir / f"frame_{int(time.time())}.jpg"; cv2.imwrite(str(pth), frame); print("Saved:", pth)
        elif k == ord('g'):      # grab still & print full probs (like test.py)
            with torch.no_grad():
